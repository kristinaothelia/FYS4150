#bare for teste ting uten aa oedelegge git...
...
def Jacobi(A, n, epsilon=1e-8):
	...
	for i in range(n):
		for j in range(i+1, n):
			#print(maxnondiag, type(maxnondiag))
			#print(abs(A[i,j]), type(abs(A[i,j])))
			#print("------------")
			#print(type(maxnondiag))
			if abs(A[i,j]) > maxnondiag:
				maxnondiag = abs(A[i,j])
...
if __name__ == "__main__":
	...
	diag = 2/h**2      # d, diagonal
	non_diag = -1/h**2  # a, above/beneath diagonal
