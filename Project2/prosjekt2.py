...
def Jacobi(A, n, epsilon=1e-8, max_it=1e4):
	...
	while (maxnondiag > epsilon) and (iterations <= max_it):
		...
		maxnondiag, k, l = MaxNonDiag(A,n) # more correct here, right?
		iterations += 1
...
if __name__ == "__main__":
	...
	N = 400              # matrix dimension (N=4 does not work. Need to fix?) Quantum: 400
	max_it = 2*N**2     # max iterations

	rho0 = 0            # rho min
	rhoN = 10            # rho max  morten uses 10 in 2d, why??
	...
	##### MÅ VI OGSÅ HA ARMADILLO, ELLER HOLDER NUMPY? #####
	...
	if BucklingBeam:
		...
		# Calculate analytical eigenpairs, always sorted????????
	...
	if QuantumMechanics:
		...
		# exercise 2e, 'helium'?
			...
			omega_list = [0.01, 0.5, 1, 5]  # input with default list maybe?
			EigVec_lowest = []
			...
			for w in range(len(omega_list)):
				...
				# only interested in lowest state???
				print(EigVal_np[0])
				EigVec_lowest.append(EigVec_np[:,0])


			labels = [r'$\omega =0.01$', r'$\omega =0.5$', r'$\omega =1$', r'$\omega =5$']
